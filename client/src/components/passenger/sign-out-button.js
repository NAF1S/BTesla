...
import { signOut } from "@/lib/passenger-api";
 ...
 * passenger has just signed out of. `refresh` re-renders the server components,
 * which is what makes the layout stop showing them as signed in.
 *
 * A failure is not worth blocking on: if the request could not reach the API, the
 * cookie is still there, but the honest thing is to send the passenger to the
 * sign-in screen and let the guard there decide. Swallowing the error silently
 * would leave somebody staring at a button that appeared to do nothing.
 */
